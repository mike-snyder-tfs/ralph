...
class SecurityScanAPITests(RalphAPITestCase):
    ...
    def test_get_security_scan_for_ip(self):
        ...
        self.security_scan1 = SecurityScanFactory(base_object=ip1.ethernet.base_object)
        ip2 = IPAddressFactory(address="192.168.128.2")
        self.security_scan2 = SecurityScanFactory(base_object=ip2.ethernet.base_object)
    ...
    def test_get_security_scan_details(self):
        ...
        for field in ["details_url", "rescan_url"]:
            self.assertEqual(response.data[field], getattr(self.security_scan, field))
    ...
    def test_patch_security_scan(self):
        ...
        vulnerability = VulnerabilityFactory()
        data = {
            "last_scan_date": (datetime.now() + timedelta(days=10))
            .replace(microsecond=0)
            .isoformat(),
            "scan_status": ScanStatus.error.name,
            "next_scan_date": (datetime.now() + timedelta(days=15))
            .replace(microsecond=0)
            .isoformat(),
        ...
        self.assertEqual(
            self.security_scan.last_scan_date.isoformat(), data["last_scan_date"]
        )
        self.assertEqual(self.security_scan.scan_status, ScanStatus.error)
        self.assertEqual(
            self.security_scan.next_scan_date.isoformat(), data["next_scan_date"]
        )
    ...
    def test_posting_scan_replace_old_one_when_scan_already_exists(self):
        ...
        response = self.client.post(reverse("securityscan-list"), data, format="json")
...
class VulnerabilityAPITests(RalphAPITestCase):
    ...
    def test_get_vulnerability_details(self):
        ...
        self.assertEqual(
            ...
            self.vulnerability.patch_deadline.replace(microsecond=0).isoformat(),
        )
    ...
    def test_create_vulnerability(self):
        ...
        self.assertEqual(
            vulnerability.external_vulnerability_id, data["external_vulnerability_id"]
        )
    ...
    def test_patch_vulnerability(self):
        ...
        data = {
            ...
            "patch_deadline": (self.vulnerability.patch_deadline + timedelta(days=3))
            .replace(microsecond=0)
            .isoformat(),
            "risk": Risk.high.name,
            "external_vulnerability_id": self.vulnerability.external_vulnerability_id
            + 10,  # noqa
        }
        response = self.client.patch(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            self.vulnerability.patch_deadline.isoformat(), data["patch_deadline"]
        )
...
class TestExternalVulnerability(RalphAPITestCase):
    def setUp(self):
        ...
        self.data = {
            ...
            "next_scan_date": "2016-01-01T00:00:00",
            "host_ip": ip.address,
            "external_vulnerabilities": [self.vulnerability.external_vulnerability_id],
        }
    ...
    def test_create_scan_by_external_id_works(self):
        ...
        security_scan = SecurityScan.objects.get(pk=response.data["id"])
        self.assertEqual(self.vulnerability.id, security_scan.vulnerabilities.get().id)
